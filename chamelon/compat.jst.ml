...
let mkArg ?id:(sort = Jkind.Sort.value) e = Arg (Targ_expr (e, sort))

let map_arg_or_omitted f arg =
  match arg with Arg (Targ_expr (e, sort)) -> Arg (Targ_expr (f e, sort)) | o -> o

let fold_arg_or_omitted f init arg =
  match arg with Arg (Targ_expr (e, sort)) -> f init (e, sort) | _ -> init

let option_of_arg_or_omitted arg =
  match arg with Arg (Targ_expr (e, sort)) -> Some (e, sort) | _ -> None
